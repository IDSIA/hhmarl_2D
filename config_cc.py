...
class Config(object):
    def __init__(self):
        ...
        parser.add_argument('--agent_mode', type=str, default="escape", help='Agent mode: Fight or Escape')
        ...
        parser.add_argument('--rew_type', type=str, default='dynamic', help='Reward type')
        parser.add_argument('--sequential', type=bool, default=False, help='Reward type')

        parser.add_argument('--horizon', type=int, default=150, help='Length of horizon')
        parser.add_argument('--level', type=int, default=3, help='Length of horizon')
    ...
    def set_metrics(self):
        ...
        if self.args.ss == 3:
            self.args.obs_dim = 39

        self.args.log_path = os.path.join(os.path.dirname(__file__), 'results', self.args.exp_folder, self.args.log_name) if self.args.exp_folder else os.path.join(os.path.dirname(__file__), 'results', self.args.log_name)

        if self.args.restore:
            if self.args.restore_path is None:
                if self.args.exp_folder:
                    self.args.restore_path = os.path.join(os.path.dirname(__file__), 'results', self.args.exp_folder, f'SS{self.args.ss}_L{self.args.level-1}{exp_name}', 'checkpoint')
                    #self.args.restore_path = os.path.join(os.path.dirname(__file__), 'results', self.args.exp_folder, f'SS{self.args.ss}_L{self.args.level}{exp_name}_SAVE', 'checkpoint')
                else:
                    self.args.restore_path = os.path.join(os.path.dirname(__file__), 'results', f'SS{self.args.ss}_L{self.args.level-1}{exp_name}', 'checkpoint')
