...
class DogfightScenario(MultiAgentEnv):
    ...
    def step(self, action):
        ...
        return self._state_hier(), self.commander_rewards, terminateds, truncateds, {}
    ...
    def _sub_steps(self, commander_action):
        ...
        while s <= self.n_sub_steps and not event:
            ...
            for i in range(1, self.total_num+1):
                if self.sim.unit_exists(i):
                    ...
                    if i <= self.num_agents:
                        actions = self._sp_actions(i, u, u.ac_type, policy_type="escape" if commander_action[i]==0 else "fight", commander=commander_action[i]-1)
                    else:
            ...
            if not event:
                #min. 5 or min. 10 (at critical pos.) low-level steps
    ...
    def _situation_reward(self, commander):
        def eval_attack(ag_id, opp_id, comm=None):
            if self.surrounding_dists[ag_id][comm if comm else 0] < 0.1:
        ...
        for i in range(1, self.num_agents+1):
            if self.closest_opps[i] and commander[i] > 0:
                try:
    ...
    def _surrounding_event(self):
        def eval_event(ag_id, opp_id, idx):
            if self.surrounding_dists[ag_id][idx] < 0.1:
    ...
    def _get_policy(self, policy_type):
        if policy_type=="fight":
            ModelCatalog.register_custom_model(f"ac1_model_l5",Fight1)
            ModelCatalog.register_custom_model(f"ac2_model_l5",Fight2)
            try:
                check_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'results', 'Level5_fight_2vs2', 'checkpoint')
                return Policy.from_checkpoint(check_path, ["ac1_policy", "ac2_policy"])
            except:
                raise NameError('Could not find L5 Fight Policy. Store in results/Level5_fight_2vs2/checkpoint')
        else:
